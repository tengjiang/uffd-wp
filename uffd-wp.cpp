...
// Current problem: mprotect failed with larger page sizes, need to raise the limit in config
...
uint64_t random_write_experiment(bool use_uffd, int number_writes, int clean_interval = 0)
{
    ...
    if (clean_interval > 0)
    {
        ...
        while (remaining_writes > 0)
        {
            ...
            if (use_uffd)
            {
                // TODO: Hacky, Get the fd another way.
...
int main()
{
    ...
    printf("UFFD elapsed: %lu us\n", elapsed_uffd);
    printf("SEGV elapsed: %lu us\n", elapsed_segv);
    ...
    printf("All experiments concluded.\n");
    return 0;
}
